...
const downloadContentfulSchema = z
  .object({
    fields: z.object({
      title: z.string(),
      description: z.string(),
...
export function contentfulDownloadLoader(): Loader {
  return {
    ...
    async load({ logger, store, parseData }) {
      ...
      const downloads = await client.getEntries({
        content_type: "download",
        limit: 1000,
      });

      for (const download of downloads.items) {
        const parsedDownload = downloadContentfulSchema.safeParse(download);

        if (!parsedDownload.success) {
          throw new Error("Failed to parse download API response");
        }
        const { data } = parsedDownload;
        const parsedData = await parseData({
          id: data.slug,
          data,
        });
